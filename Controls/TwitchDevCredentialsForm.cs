...
public sealed class TwitchDevCredentialsForm : WadevoDialogForm
{
    private readonly TextBox _clientIdTextBox = new();
    private readonly TextBox _clientSecretTextBox = new();

    public TwitchDevCredentialsForm() : base("Twitch App Setup")
    {
        ...
        saveButton.ButtonClicked += (_, _) =>
        {
            ...
            {
                WadevoMessageBox.Show(this, "Both fields are required.", "Twitch App Setup");
                return;
            }

            new TwitchDevCredentialsStore().Save(clientId, clientSecret);
