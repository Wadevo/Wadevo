...
public sealed class TwitchOAuthCallbackListener
{
    public async Task<TwitchOAuthCallbackResult> WaitForCallbackAsync(
        string redirectUri,
        CancellationToken cancellationToken = default)
    {
        ...
        string error = context.Request.QueryString["error_description"]
            ?? context.Request.QueryString["error"]
            ?? "";

        TwitchOAuthCallbackResult result = string.IsNullOrWhiteSpace(error)
            ? TwitchOAuthCallbackResult.FromSuccess(code, state)
            : TwitchOAuthCallbackResult.FromError(error, state);
    ...
    private static async Task WriteBrowserResponseAsync(
        ...
        TwitchOAuthCallbackResult result)
    {
        string message = result.Success
            ? "Twitch connected. You can return to Wadevo."
            : "Twitch connection failed. You can return to Wadevo.";

        string html =
            "<!doctype html>" +
            "<html><head><title>Wadevo Twitch Login</title></head>" +
